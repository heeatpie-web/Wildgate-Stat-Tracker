...
import { Trash2, Edit2, Pin, ChevronDown, ChevronUp, Clock, Image as ImageIcon, Download, ArrowUpDown, Users, Swords, X } from 'lucide-react';
                  ...
                  <tr key={m.id} onClick={() => setSelectedMatchForDetails(m)} className={`border-b border-md-sys-outline/5 transition-colors group ${rowBg} hover:brightness-125 cursor-pointer`}>
                    ...
                    <td className="p-4 text-right">
                        <div className="flex justify-end items-center gap-1" onClick={e => e.stopPropagation()}>
                            <button onClick={() => onPin(m.id)} className={`p-2 rounded-lg transition-all opacity-30 group-hover:opacity-100 ${m.isPinned ? 'bg-yellow-500 text-black !opacity-100' : 'hover:bg-md-sys-surface3 text-md-sys-on-surface/40'}`}><Pin size={14} className={m.isPinned ? 'fill-black' : ''}/></button>
