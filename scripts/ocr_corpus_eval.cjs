...
function filterActiveUser(list, activeUser) {
  const activeKey = digitFold(canonicalizeName(activeUser));
  if (!activeKey) return safeArray(list);
  return safeArray(list).filter((entry) => (
    digitFold(canonicalizeName(entry)) !== activeKey
  ));
}
...
function main() {
  ...
  for (const t of truthSamples) {
    ...
    const teammateStats = setStats(
      ...
      filterActiveUser(p.teammates, args.activeUser),
      canonicalizeName
    );
