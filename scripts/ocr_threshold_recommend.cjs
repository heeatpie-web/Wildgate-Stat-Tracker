...
function main() {
  ...
  const recallAvg = avg([
    summary.teammateRecall,
    summary.opponentRecall,
    summary.modifierRecall,
  ], 0.7);
  const precisionAvg = avg([
    summary.teammatePrecision,
    summary.opponentPrecision,
    summary.modifierPrecision,
  ], recallAvg);
  const usability = Number.isFinite(summary.sessionUsablePassRate)
    ? Number(summary.sessionUsablePassRate)
    : recallAvg;
  const grouping = Number.isFinite(summary.teamGroupingAccuracy)
    ? Number(summary.teamGroupingAccuracy)
    : 0.75;
