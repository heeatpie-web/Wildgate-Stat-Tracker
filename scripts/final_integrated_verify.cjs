...
async function simulate(imagePath) {
    try {
        console.log(`\n=== COMPREHENSIVE TEST: ${imagePath.split('\\').pop()} ===`);
        ...
        const filtered = players.filter(p => {
            ...
            if (['MURDER', 'SPAGHURDER', 'MEANR', 'THAN', 'AVG'].includes(up)) return false;
            return true;
        });

        if (unassociated.length > 0) {
            const shipName = unassociated.map(s => s.text).join(' ');
            if (shipName.length > 5) filtered.push({ name: "Unknown Player (Non-ASCII?)", shipType: shipName });
        }
