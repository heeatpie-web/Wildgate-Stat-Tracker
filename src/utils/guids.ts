/**
 * @module guids
 * Hardcoded GUID to display-name mappings for heroes, ships, weapons,
 * equipment, and perks. Unknown GUIDs are tracked by
 * createMappingSlice.registerUnknownId() for user resolution.
 */
const withGuidCaseAliases = (entries: Record<string, string>): Record<string, string> => {
    const out: Record<string, string> = {};
    Object.entries(entries).forEach(([guid, name]) => {
        out[guid] = name;
        out[guid.toLowerCase()] = name;
        out[guid.toUpperCase()] = name;
    });
    return out;
};

export const HERO_GUIDS: Record<string, string> = withGuidCaseAliases({
    'C0C3960248AD43D20AA6DDA8AEB81424': 'Sal',
    'E7539B7C4483C338E55B15B102E2F006': 'Adrian',
    'E305EC254F7CD5BA4107A381E9BEA287': 'Venture',
    '4BB6782A475B3FBA76ECA683DECBC23B': 'Kae',
    '39BEFBAF436F529669C207B5106D256A': 'Mophs',
    'D978177246E41E1DC58185AD36ADB864': 'Charlie',
    '282D534546A0DC63DDCE24A388C53B64': 'Ion',
    '130A8C544E4A13DBC33C9CBB47D4B36B': 'Sammo',
});

export const SHIP_GUIDS: Record<string, string> = withGuidCaseAliases({
    '0BFFF89B44027290DC6348B95A6B0F11': 'Hunter',
    'B16EF7D94FC7AE05ACA2999B2023138F': 'Hunter',
    '238FE96442789BC0C2E416BBDFDBCC52': 'Scout',
    '9299137344DC982A469564BDAD18711C': 'Solo Outlaw',
    '5F6F4E8647A82086A1DB2C8566E100DA': 'Outlaw',
    '9EC8F663442D6D70654E72A69AAE9912': 'Battle Scout',
    'C42238F241BB534AE55AF6BB7C88DDD9': 'Bastion',
});

export const WEAPON_GUIDS: Record<string, string> = withGuidCaseAliases({
    'EEACC46745DD1CBCB28878B34BE78BDC': 'AIM-e',
    'E12964DF4EB63ED2C1A33888DA5E8C60': 'MK2 Blaster',
    'F350FD964B4A0E59F068AE88D6D9650C': 'The Doctor',
    '96E8EBE7458D6614EDCE83B6561C5FAE': 'Double Whammy',
    'D6936A484DB111488285BC82D1841483': 'Sidelong',
    'F2A6D62B49145170B1CFE092C9F057CD': 'Resonator',
    '7B68346F45753D427AA78D961CDB45AF': 'Beam Rifle',
    'FA1A98164D7E396731D2CEBCA07C5A1E': 'Painter',
    '0AAA474246ADEDFF6D39FC80D9794461': 'Rocket Launcher',
    '2CACFD654B27AB15B6AC5FBB18216117': 'Hand Cannon',
    '13638B5748A18C0CC643E1830A5A88DB': 'Foam Sprayer',
    '77A11BF74EE089D011376584479CFEA2': 'Surefire',
    'CF3DFA6A424F043F7151BCB8BBCFA48A': 'Surefire',
    'D67BB8DA4C46726739FDBC887F37A9C1': 'Sonic Boom',
});

export const EQUIPMENT_GUIDS: Record<string, string> = withGuidCaseAliases({
    'F2B54FEC47BBDBEA641EB9AD846A0A8D': 'Repair Drone',
    'D758D49F45005A77CB13ABAE81E204EB': 'Repulsor',
    '6EEA22004EA2D98A46779A9592E0C4B2': 'Teleport Reloader',
    '7331F55044069CD1196C43883CD12EDF': 'Shield Wall',
    '990B68A64D67094FADE3D3B4E229D6F4': 'Rock!',
    '1FC6C97040714EF444F7119B75377054': 'Rock!',
    '58C97F0F43E24F47057BE7AE9321AC7D': 'Healing Drone',
    '5D1B66E148D64162F74218B269E68016': 'Foam Can',
    '93071BFC425FC379C95B12961147AF9B': 'Plasma Can',
    '3E9EF2344C50F8026CDCAAAAF7777907': 'Attack Drone',
    '20C5C5A04C5A86EFAF1F9FAF2C0DD60C': 'Drill Charge',
    'A55363B942D501960129419A7D323AE6': 'Flash Shield',
    '084F9E4B4A0F2C7BCC525C9B4C94C318': 'Impact Can',
    '514C1B4747ED7E26AF209D9F5B7CED24': 'Sensor Trap',
    '13082B1E4491D0640FCB7689B306FEEB': 'Blast Can',
    'CD21C7B2468EC990E4AFDE8B27CFE398': 'Adventure Gear',
});

export const PERK_GUIDS: Record<string, string> = withGuidCaseAliases({
    'A7291E13434D4D67CFEAD0928F4CEA69': 'Boarder',
    '784E54BC47645CB5347E50A2544FD57D': 'Defender',
    '453697DC44F57CACA1F4BABC6171F3AD': 'Efficient Resourcing',
    '16CFDB1240084B87FBF8BA8D654C446D': 'Engineering',
    '67D507DF47BB41C6EE3DC5A4EA46B458': 'Sal Inventor',
    'DC33FBDD43EC6B059ADFCC931279E204': 'Sammo Defender',
    '14B840004BBC83F4A85CDFA9DF70AF0E': 'Kae Teleport',
    '77C7FADD4BE453847A719F923BA17737': 'Ion Smash',
    '54CA32F14C379EF35D17E4AA375BD65E': 'Venture Explorer',
    '5BE02211442D38DC8EE02784F0D736A2': 'Salvager',
    'F6547E4F4F949CD68E93878D5003DE5F': 'Mad Bomber',
    '5EBC4A5C493E31F4AC121B897E8CAE0A': 'Turbine Factory',
});
