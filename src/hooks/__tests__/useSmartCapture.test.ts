...
describe('useSmartCapture', () => {
  ...
  it('processStoredImage marks capture processed and stages pending data', async () => {
    ...
    vi.mocked(rerunOCROnArtifact).mockResolvedValue({
      ...
      data: {
        ...
        playerShip: { shipType: 'Hunter (4 Player)', confidence: 90, rawText: 'Hunter' } as any,
        playerTeamName: '',
        reachModifiers: [],
        enemyShips: [],
        teammates: [{ name: 'Wingman', confidence: 88, isTeammate: true, rawText: 'Wingman' } as any],
        opponentTeams: [],
  ...
  it('promotes stable teammate names with temporal fusion when enhancement is enabled', async () => {
    ...
    const makeData = (confidence: number) => ({
      ...
      reachModifiers: [],
      enemyShips: [],
      teammates: [{ name: 'Wingman', confidence, isTeammate: true, rawText: 'Wingman' } as any],
      opponentTeams: [],
  ...
  it('uses historical profile names as background OCR candidates even when roster is empty', async () => {
    ...
    vi.mocked(rerunOCROnArtifact).mockResolvedValue({
      ...
      data: {
        ...
        reachModifiers: [],
        enemyShips: [],
        teammates: [{ name: 'BarbiePrirne', confidence: 76, isTeammate: true, rawText: 'BarbiePrirne' } as any],
        opponentTeams: [],
  ...
  it('uses bundled OCR lexicon names as a background fallback when local history is empty', async () => {
    ...
    vi.mocked(rerunOCROnArtifact).mockResolvedValue({
      ...
      data: {
        ...
        reachModifiers: [],
        enemyShips: [],
        teammates: [{ name: 'Ask4o', confidence: 76, isTeammate: true, rawText: 'Ask4o' } as any],
        opponentTeams: [],
        overallConfidence: 76,
        captureTimestamp: Date.now(),
      },
