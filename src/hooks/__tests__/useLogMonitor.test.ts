...
describe('useLogMonitor', () => {
  ...
  it('dispatches a one-shot auto-capture event when in-game telemetry is observed in dev mode', async () => {
    const nowSec = Math.floor(Date.now() / 1000);
    ...
    const createdDraft = addMatch.mock.calls[0]?.[0] as { id?: number } | undefined;
      ...
      ipcCallbacks['log-data']?.([
        {
          ...
          ClientTimestamp: nowSec + 5,
        },
      ]);
    });

    let autoCaptureEvents = dispatchSpy.mock.calls
      .map(([evt]) => evt as Event)
      .filter((evt) => evt.type === 'telemetry:draft-ingame-auto-capture') as CustomEvent[];
    expect(autoCaptureEvents).toHaveLength(1);
    expect(autoCaptureEvents[0]?.detail).toEqual({ matchId: createdDraft?.id });
      ...
      ipcCallbacks['log-data']?.([
        {
          ...
          ClientTimestamp: nowSec + 6,
        },
      ]);
    });

    autoCaptureEvents = dispatchSpy.mock.calls
      .map(([evt]) => evt as Event)
      .filter((evt) => evt.type === 'telemetry:draft-ingame-auto-capture') as CustomEvent[];
    expect(autoCaptureEvents).toHaveLength(1);
    dispatchSpy.mockRestore();
