      ...
      <div className="bg-md-sys-surface1 w-full max-w-2xl rounded-[32px] p-8 shadow-2xl border border-md-sys-outline/20 flex flex-col gap-6 max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-md-sys-surface2 p-4 rounded-2xl">
            ...
            <div className="grid grid-cols-2 gap-2">
              {SYSTEMS.map(s => (
                <div key={s} className={`flex justify-between items-center p-2 rounded-xl border border-md-sys-outline/5 ${(editedMatch.weapons?.[s] || 0) > 0 ? 'bg-blue-500/20 border-blue-500/30' : 'bg-md-sys-surface1'}`}>
                  <div className="flex flex-col ml-1">
                  ...
                  <div className="flex gap-1 items-center">
                    <button onClick={() => setEditedMatch(prev => ({ ...prev, weapons: { ...(prev.weapons || {}), [s]: Math.max(0, (prev.weapons?.[s] || 0) - 1) } }))} className="w-6 h-6 rounded flex items-center justify-center font-bold bg-md-sys-surface3 hover:bg-md-sys-surface1">-</button>
                    <button onClick={() => setEditedMatch(prev => ({ ...prev, weapons: { ...(prev.weapons || {}), [s]: (prev.weapons?.[s] || 0) + 1 } }))} className="w-6 h-6 rounded flex items-center justify-center font-bold bg-blue-500 text-white hover:brightness-110">+</button>
                  </div>
                </div>
              ))}
            </div>
