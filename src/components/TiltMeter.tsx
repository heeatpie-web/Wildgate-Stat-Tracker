...
export const TiltMeter: React.FC<TiltMeterProps> = ({ recentMatches }) => {
    ...
    if (!hasData) explanation = "No Data";
    else if (tiltScore > 80) explanation = "CRITICAL TILT";
        ...
        <div className="md3-card p-6 rounded-xl flex items-center gap-6 shadow-lg border border-md-sys-outline/10">
            <div className="relative w-24 h-24 flex-shrink-0">
                <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                        ...
                        strokeWidth="8"
                        fill="transparent"
                        strokeDasharray={`${hasData ? tiltScore * 2.51 : 0} 251`}
                        className={!hasData ? "text-md-sys-on-surface/30" : tiltScore > 80 ? "text-danger" : "text-success"}
                    />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-black">{hasData ? `${tiltScore}%` : '--'}</span>
            ...
            <div className="flex-1">
                ...
                <div className="text-label-sm opacity-60 leading-relaxed font-medium">
                    {!hasData
                        ? "No matches in the selected scope yet, so tilt cannot be calculated."
                        : tiltScore > 50
