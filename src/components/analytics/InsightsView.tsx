        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className={`grid gap-4 ${dense ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1 md:grid-cols-2'}`}>
                        {relationshipInsights.map((rel, i) => (
                            <div key={i} onClick={() => onDrillDown(rel.playerName, rel.type === 'ally' ? 'Teammate' : 'Opponent')}
                                className={`md3-card rounded-2xl relative overflow-hidden hover:scale-[1.02] transition-transform cursor-pointer ${dense ? 'p-4' : 'p-6'}`}>
            ...
            <div className={`grid gap-4 pb-4 ${dense ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'}`}>
                {enrichedInsights.map((stat, i) => (
                    <div key={i} className={`md3-card relative overflow-hidden hover:scale-[1.02] transition-transform cursor-pointer group rounded-2xl ${dense ? '!p-6' : '!p-8'}`}>
                        ...
                        <div className="text-[10px] font-bold px-2 py-1 md3-surface-low rounded-lg inline-block">{stat.subValue}</div>
                    </div>
                ))}
