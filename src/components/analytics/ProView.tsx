...
import { generateProEditorial } from '../../utils/analyticsEditorial';
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar animate-fade-in p-1">
                ...
                <div className="flex gap-2">
                    {['All', 'Win', 'Loss'].map(type => (
                        <button key={type} onClick={() => setProResultType(type as any)}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${proResultType === type ? 'bg-md-sys-primary text-md-sys-onPrimary' : 'bg-md-sys-surface1 hover:bg-md-sys-surface3'}`}>{type}</button>
                    ))}
                </div>
            ...
            {!dense && (
                <div className="bg-md-sys-surface2 rounded-2xl border border-white/5 p-6">
                    <p className="text-sm leading-relaxed opacity-70">{generateProEditorial(matches)}</p>
                </div>
            )}
            ...
            <div className={`grid gap-4 ${dense ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
                <div className={`bg-md-sys-surface2 rounded-2xl flex flex-col border border-white/5 ${dense ? 'p-6 min-h-[300px]' : 'p-8 min-h-[400px]'}`}>
                    <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-sm' : 'text-base'}`}><Rocket size={16} /> Ship Distribution</h3>
                ...
                <div className={`bg-md-sys-surface2 rounded-2xl flex flex-col border border-white/5 ${dense ? 'p-6 min-h-[300px]' : 'p-8 min-h-[400px]'}`}>
                    <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-sm' : 'text-base'}`}><User size={16} /> Hero Distribution</h3>
                    <ResponsiveContainer width="100%" height="100%">
                ...
                <div className={`bg-md-sys-surface2 rounded-2xl flex flex-col border border-white/5 ${dense ? 'p-6 min-h-[300px]' : 'p-8 min-h-[400px]'}`}>
                    <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-sm' : 'text-base'}`}><BarChart3 size={16} /> {metricTitle}</h3>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={metricData} layout="vertical" margin={{ left: 20 }}>
                            <XAxis type="number" hide /><YAxis dataKey="name" type="category" width={80} tick={{ fontSize: 10, fontWeight: 'bold', fill: '#888' }} axisLine={false} />
                            ...
                            <Bar dataKey="value" radius={[0, 4, 4, 0]}>{metricData.map((entry, index) => <Cell key={`cell-${index}`} fill={getColor(entry.name)} />)}<LabelList dataKey="value" position="right" style={{ fontSize: 10, fontWeight: 'bold', fill: 'var(--md-sys-color-on-surface)' }} /></Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
                <div className={`bg-md-sys-surface2 rounded-2xl flex flex-col border border-white/5 ${dense ? 'p-6 min-h-[300px]' : 'p-8 min-h-[400px]'}`}>
                    <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-sm' : 'text-base'}`}><TrendingUp size={16} /> {metricTitle} Trend</h3>
                    <ResponsiveContainer width="100%" height="100%">
