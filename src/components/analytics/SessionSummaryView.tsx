        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar animate-fade-in p-1">
            ...
            {!dense && (
                <div className="md3-card rounded-2xl p-6">
                    <p className="text-body leading-relaxed opacity-60">{generateSessionSummaryEditorial(data)}</p>
                </div>
            )}

            {/* Today's session */}
            <div className={`md3-card rounded-2xl ${dense ? 'p-4' : 'p-8'}`}>
                <h3 className={`font-bold uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-body' : 'text-lg'}`}>
                    <Calendar size={16} /> Today's Session
                </h3>
                {today ? (
                    <div className={`grid gap-4 ${dense ? 'grid-cols-4' : 'grid-cols-2 md:grid-cols-4'}`}>
                        <div>
                            <div className="text-label-sm font-bold uppercase tracking-widest opacity-60 mb-1">Record</div>
                        ...
                        <div>
                            <div className="text-label-sm font-bold uppercase tracking-widest opacity-60 mb-1">Win Rate</div>
                            <div className={`font-black ${today.winRate >= 50 ? 'text-success' : 'text-danger'} ${dense ? 'text-2xl' : 'text-3xl'}`}>{today.winRate}%</div>
                        </div>
                        <div>
                            <div className="text-label-sm font-bold uppercase tracking-widest opacity-60 mb-1">Total Kills</div>
                            <div className={`font-black text-md-sys-primary ${dense ? 'text-2xl' : 'text-3xl'}`}>{today.totalKills}</div>
                        </div>
                        <div>
                            <div className="text-label-sm font-bold uppercase tracking-widest opacity-60 mb-1">Best Streak</div>
                            <div className={`font-black text-warning ${dense ? 'text-2xl' : 'text-3xl'}`}>{today.bestStreak}</div>
                        </div>
                    </div>
                ) : (
                    <div className={`text-center opacity-40 font-bold uppercase ${dense ? 'py-6 text-label-sm' : 'py-12 text-body'}`}>No matches played today</div>
                )}
            </div>
            ...
            {yesterday && today && (
                <div className={`md3-card rounded-2xl ${dense ? 'p-4' : 'p-6'}`}>
                    <h3 className={`font-bold uppercase opacity-60 mb-3 flex items-center gap-2 ${dense ? 'text-label-sm' : 'text-body'}`}>
                    ...
                    <div className={`grid gap-4 ${dense ? 'grid-cols-3' : 'grid-cols-1 md:grid-cols-3'}`}>
                        {[
                            ...
                            <div key={item.label} className="flex items-center justify-between">
                                <div>
                                    <div className="text-label-xs font-bold uppercase opacity-40">{item.label}</div>
                                    <div className="text-lg font-bold">{item.today}</div>
                                </div>
                                <div className={`text-body font-bold ${item.delta > 0 ? 'text-success' : item.delta < 0 ? 'text-danger' : 'opacity-40'}`}>
            ...
            <div className={`md3-card rounded-2xl ${dense ? 'p-4' : 'p-6'}`}>
                ...
                {data.last7Days.length === 0 ? (
                    <div className="text-center opacity-40 font-bold uppercase py-6 text-label-sm">No recent data</div>
                ) : (
                    <div className="flex flex-col gap-2">
                        {data.last7Days.map((day) => (
                            <div key={day.date} className="flex items-center justify-between py-2 border-b last:border-0">
                                <div className="text-label-sm font-bold opacity-60">{new Date(day.date + 'T12:00:00').toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</div>
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-label-sm font-bold opacity-40 min-w-[50px] text-right">{day.totalKills} kills</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className={`md3-card rounded-2xl ${dense ? 'p-4' : 'p-6'}`}>
                ...
                <div className={`grid gap-4 ${dense ? 'grid-cols-3' : 'grid-cols-1 md:grid-cols-3'}`}>
                    <div>
                        <div className="text-label-sm font-bold uppercase opacity-40">Matches / Day</div>
                        <div className="text-xl font-bold text-md-sys-primary">{data.dailyAverage.matches}</div>
                    </div>
                    <div>
                        <div className="text-label-sm font-bold uppercase opacity-40">Wins / Day</div>
                        <div className="text-xl font-bold text-success">{data.dailyAverage.wins}</div>
                    </div>
                    <div>
                        <div className="text-label-sm font-bold uppercase opacity-40">Kills / Day</div>
