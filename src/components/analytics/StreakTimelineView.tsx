    ...
    return (
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar animate-fade-in p-1">
            ...
            <div className={`grid gap-4 ${dense ? 'grid-cols-4' : 'grid-cols-2 md:grid-cols-4'}`}>
                <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-3' : 'p-6'}`}>
                ...
                {chartData.length < 2 ? (
                    ...
                    <ResponsiveContainer width="100%" height={dense ? 250 : 350}>
                        <AreaChart data={chartData}>
                            ...
                            <Area type="stepAfter" dataKey="streak" stroke="#22c55e" strokeWidth={2} fill="url(#streakGreen)"
                                activeDot={{ fill: '#22c55e', stroke: 'var(--md-sys-color-surface1)', strokeWidth: 2 }} />
                        </AreaChart>
                    </ResponsiveContainer>
                )}
            </div>
