        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar animate-fade-in p-1">
            ...
            {!dense && (
                <div className="bg-md-sys-surface2 rounded-2xl border border-white/5 p-6">
                    <p className="text-sm leading-relaxed opacity-70">{generateTimePatternEditorial(data)}</p>
                </div>
            )}
            ...
            <div className={`grid gap-4 ${dense ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'}`}>
                <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-4' : 'p-6'}`}>
                    <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Peak Hour</div>
                    <div className={`font-black text-md-sys-primary ${dense ? 'text-3xl' : 'text-4xl'}`}>{data.peakHour}:00</div>
                    <div className="text-[10px] font-bold opacity-40 mt-1">You play the most at this hour</div>
                </div>
                <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-4' : 'p-6'}`}>
                    <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Peak Day</div>
                    <div className={`font-black text-md-sys-primary ${dense ? 'text-3xl' : 'text-4xl'}`}>{DAY_NAMES[data.peakDay]}</div>
                    <div className="text-[10px] font-bold opacity-40 mt-1">Your busiest day of the week</div>
                </div>
            </div>

            {/* Hour-of-day chart */}
            <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-4 min-h-[250px]' : 'p-6 min-h-[350px]'}`}>
                <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-xs' : 'text-sm'}`}><Clock size={14} /> Matches by Hour</h3>
                <ResponsiveContainer width="100%" height={dense ? 200 : 280}>
                    <BarChart data={hourData}>
                        <CartesianGrid strokeOpacity={0.05} vertical={false} />
                        <XAxis dataKey="name" tick={{ fontSize: 9 }} interval={dense ? 2 : 1} />
            ...
            {/* Day-of-week chart */}
            <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-4 min-h-[250px]' : 'p-6 min-h-[350px]'}`}>
                <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-xs' : 'text-sm'}`}><Clock size={14} /> Win Rate by Day</h3>
                <ResponsiveContainer width="100%" height={dense ? 200 : 280}>
            ...
            {/* Heatmap */}
            <div className={`bg-md-sys-surface2 rounded-2xl border border-white/5 ${dense ? 'p-4' : 'p-6'}`}>
                <h3 className={`font-black uppercase opacity-60 mb-4 flex items-center gap-2 ${dense ? 'text-xs' : 'text-sm'}`}><Clock size={14} /> Activity Heatmap</h3>
                <div className="overflow-x-auto">
                    <div className="grid grid-cols-[auto_repeat(24,1fr)] gap-0.5 min-w-[600px]">
                        <div></div>
                        {Array.from({ length: 24 }, (_, h) => (
                            <div key={h} className="text-[8px] font-bold text-center opacity-40">{h}</div>
                        ))}
                        {DAY_NAMES.map((day, d) => (
                            <React.Fragment key={d}>
                                <div className="text-[9px] font-black opacity-60 pr-2 flex items-center">{day}</div>
                                {Array.from({ length: 24 }, (_, h) => {
                                    ...
                                    return (
                                        <div key={h} className="aspect-square rounded-sm relative group"
                                            ...
                                            {cell && cell.matches > 0 && (
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-black/80 text-white text-[9px] p-1.5 rounded-lg whitespace-nowrap hidden group-hover:block z-50">
