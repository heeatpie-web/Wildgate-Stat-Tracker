    ...
    return (
        <div className="fixed top-24 right-6 w-[400px] h-[600px] md3-card rounded-3xl shadow-2xl border border-md-sys-outline/20 flex flex-col overflow-hidden z-[5000] animate-slide-up mg-blur bg-md-sys-surface-container-high/90">
            ...
            <div className="px-4 py-3 border-b border-md-sys-outline/10 flex justify-between items-center bg-md-sys-surface-container-lowest/50">
                <span className="text-[10px] font-black uppercase tracking-tighter opacity-80 flex items-center gap-2 text-md-sys-on-surface">
                ...
                <div className="flex items-center gap-2">
                    <button
                        onClick={onClear}
                        className="text-[9px] font-bold uppercase px-3 py-1.5 bg-md-sys-errorContainer/20 text-md-sys-error rounded-xl hover:bg-md-sys-error/30 transition-all active:scale-95"
                    >
                        Clear Buffer
                    </button>
                </div>
            ...
            <div className="px-4 py-2.5 bg-black/10 flex flex-col gap-1.5 border-b border-md-sys-outline/5">
                <div className="flex justify-between items-center">
                    ...
                    <span className={`text-[8px] font-mono font-bold ${logStatus.exists ? 'text-success' : 'text-md-sys-error'}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex justify-between items-center">
                        <span className="text-[8px] font-black uppercase opacity-30">Buffer</span>
                        <span className="text-[8px] font-mono opacity-60">
                            {logStatus.size ? `${(logStatus.size / 1024).toFixed(1)} KB` : '0 KB'}
                        </span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-[8px] font-black uppercase opacity-30">Last Pulse</span>
                        <span className="text-[8px] font-mono opacity-60">
                ...
                {logStatus.error && (
                    <div className="mt-1 p-1 bg-md-sys-errorContainer/20 rounded border border-md-sys-error/20 flex gap-2 items-center">
                        ...
                        <span className="text-[8px] font-mono text-md-sys-error truncate">{logStatus.error}</span>
                    </div>
                )}
            </div>

            {/* Log Stream */}
            <div className="flex-1 overflow-y-auto p-4 font-mono text-[9px] custom-scrollbar flex flex-col gap-2.5 bg-black/5">
                {logStatus.rawHead && (
                    <div className="mb-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                        <div className="text-[7px] font-black uppercase text-amber-500 mb-1">Raw Decoding Fault</div>
                ...
                {logFeed.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center opacity-20 text-center gap-2">
                        ...
                        <span className="uppercase font-black text-[10px] tracking-widest">Listening for Telemetry...</span>
                    </div>
                ) : (
                        ...
                        {logFeed.map((entry, i) => (
                            <div key={i} className="mg-surface-high p-2.5 rounded-xl border border-md-sys-outline/5 group hover:border-md-sys-primary/20 transition-all">
                                <div className="flex justify-between items-start mb-1.5">
                                    ...
                                    <span className="opacity-30 text-[8px] font-mono">{new Date(entry.ClientTimestamp * 1000).toLocaleTimeString([], { hour12: false })}</span>
                                </div>
                                <pre className="opacity-70 overflow-hidden text-ellipsis whitespace-pre-wrap leading-relaxed text-[8px]">
