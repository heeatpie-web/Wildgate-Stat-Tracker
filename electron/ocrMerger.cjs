...
function crossMergeCrewHubAndMap(crewHub, tactMap) {
  ...
  const mapOnlyTeams = mapShipsByType.filter((ship) => {
    ...
    const shipName = String(ship?.teamName || '').trim();
    ...
    return !enrichedTeams.some((team) => {
      ...
      if (
        shipName
        && teamName
        && !isPlaceholderTeamName(shipName, shipColor)
        && !isPlaceholderTeamName(teamName, teamColor)
        && fuzzyTeamNameMatch(shipName, teamName)
      ) {
        return true;
      }
