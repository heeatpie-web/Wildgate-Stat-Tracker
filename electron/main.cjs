...
} = require('./helpers/usableTelemetryHelpers.cjs');
...
async function beginAutoCaptureWindowSession() {
  ...
  if (!shouldHideWindow) {
    autoCaptureHiddenWindow = null;
    return;
  }

  autoCaptureHiddenWindow = mainWindow;
  mainWindow.hide();
  await delay(20);
}

async function endAutoCaptureWindowSession() {
  const hiddenWindow = autoCaptureHiddenWindow;
  autoCaptureHiddenWindow = null;
  if (!hiddenWindow || hiddenWindow.isDestroyed()) return;
  if (typeof hiddenWindow.showInactive === 'function') {
      ...
      if (stats && stats.mtimeMs !== lastMtime) {
        ...
        if (win) {
          ...
          if (data && !hasError) {
            ...
            const usableNewTelemetryEvents = extractUsableTelemetryEvents(newEvents);
            if (newEvents.length > 0) {
              win.webContents.send('log-data', { telemetry: newEvents });
            }
            const hasUsableTelemetry = usableNewTelemetryEvents.length > 0;
            if (hasUsableTelemetry) {
              await telemetryArchiveHelpers.archiveTelemetry(telemetryArchiveHelpers.getArchiveDir(app), newEvents);
            }

            let historyResult = { addedCount: 0, skippedCount: 0 };
            if (hasUsableTelemetry) {
              try {
                historyResult = await appendTelemetryHistoryEvents(newEvents);
