...
const MAP_HAZARD_NOISE_COMPACT_KEYS = new Set(
  Object.keys(KNOWN_HAZARDS || {})
    .map((key) => String(key || '').toUpperCase().replace(/[^A-Z]/g, ''))
    .filter(Boolean)
);
const MAP_HAZARD_NOISE_WORDS = new Set(
  Object.keys(KNOWN_HAZARDS || {})
    .flatMap((key) => String(key || '').toUpperCase().split(/[^A-Z0-9]+/g))
    .filter((token) => token && token.length >= 3)
);

function isLikelyMapNoiseTeamName(value) {
  const upper = String(value || '').toUpperCase().trim();
  if (!upper) return true;
  const compact = upper.replace(/[^A-Z]/g, '');
  const tokens = upper.split(/\s+/).filter(Boolean);
  if (MAP_HAZARD_NOISE_COMPACT_KEYS.has(compact)) return true;
  if (tokens.length > 0 && tokens.every((token) => MAP_HAZARD_NOISE_WORDS.has(token))) return true;
  if (tokens.length >= 2 && new Set(tokens).size === 1 && MAP_HAZARD_NOISE_WORDS.has(tokens[0])) return true;
  if (/KNOWN|HAZARD|FEATURE|ARTIFACT|RESOURCES|WILDGATE|SPECIAL/.test(upper)) return true;
  return false;
}
...
async function cropRegionAndOCR(
  ...
  try {
    ...
    const regionOCR = await runOCR(cropped, psm, ocrOptions);
...
async function runOCR(imageBuffer, psm = null, options = {}) {
  ...
  const detThresholdRaw = Number(options.threshold);
  const detThreshold = Number.isFinite(detThresholdRaw)
    ? Math.max(0.05, Math.min(0.9, detThresholdRaw))
    : 0.2;
  const minWordConfidenceRaw = Number(options.minWordConfidence);
  const minWordConfidence = Number.isFinite(minWordConfidenceRaw)
    ? Math.max(0, Math.min(100, minWordConfidenceRaw))
  ...
  const paddleWords = await paddleOcrBuffer(imageBuffer, { threshold: detThreshold });
...
function namesMatchFuzzyExact(a, b, maxDistance = 1) {
  const aKey = normalizeNameKey(a);
  const bKey = normalizeNameKey(b);
  if (!aKey || !bKey) return false;
  if (aKey === bKey) return true;
  const aNoTrailingDigits = aKey.replace(/\d+$/g, '');
  const bNoTrailingDigits = bKey.replace(/\d+$/g, '');
  if (aNoTrailingDigits && bNoTrailingDigits) {
    if (aNoTrailingDigits === bNoTrailingDigits) return true;
    if (levenshtein(aNoTrailingDigits, bNoTrailingDigits) <= Math.max(0, Number(maxDistance || 0))) {
      return true;
    }
  }
  return levenshtein(aKey, bKey) <= Math.max(0, Number(maxDistance || 0));
}

function collapsePrefixNearDuplicatePlayers(players) {
  if (!Array.isArray(players) || players.length === 0) {
    return { players: [], collapsed: 0 };
  }

  const byConfidence = [...players].sort((a, b) => Number(b?.confidence || 0) - Number(a?.confidence || 0));
  const kept = [];
  let collapsed = 0;

  const shouldCollapsePair = (leftName, rightName) => {
    ...
    return levenshtein(leftKey, rightKey) <= 2;
  };

  const choosePreferred = (current, candidate) => {
    const currentName = String(current?.name || '');
    const candidateName = String(candidate?.name || '');
    const currentKeyLen = normalizeNameKey(currentName).length;
    const candidateKeyLen = normalizeNameKey(candidateName).length;
    if (candidateKeyLen > currentKeyLen) {
      return {
        ...candidate,
        confidence: Math.max(Number(current?.confidence || 0), Number(candidate?.confidence || 0)),
      };
    }
    return {
      ...current,
  ...
  for (const candidate of byConfidence) {
    ...
    const existingIdx = kept.findIndex((entry) => shouldCollapsePair(entry?.name, candidateName));
    if (existingIdx < 0) {
      kept.push({
        ...candidate,
        name: candidateName,
    ...
    kept[existingIdx] = choosePreferred(kept[existingIdx], {
      ...candidate,
      name: candidateName,
    });
    collapsed += 1;
  ...
  return { players: kept, collapsed };
}

function sanitizeExtractedPlayerCollections(extractedData, activeUser) {
  if (!extractedData || typeof extractedData !== 'object') {
    return { removedImplicitSelf: 0, collapsedNearDuplicates: 0 };
  }

  const activeUserName = String(activeUser || '').trim();
  const hasActiveUser = normalizeNameKey(activeUserName).length > 0;
  let removedImplicitSelf = 0;
  let collapsedNearDuplicates = 0;

  const sanitizePlayers = (players, maxCount = 4, isTeammate = false) => {
    const asObjects = (Array.isArray(players) ? players : [])
      .map((player) => ({
    ...
    const withoutSelf = asObjects.filter((player) => {
      if (!hasActiveUser) return true;
      const match = namesMatchFuzzyExact(player?.name, activeUserName, 1);
      if (match) removedImplicitSelf += 1;
      return !match;
    });

    const dedupedExact = dedupeExtractedPlayers(withoutSelf, Math.max(maxCount, withoutSelf.length));
    const collapsed = collapsePrefixNearDuplicatePlayers(dedupedExact);
    collapsedNearDuplicates += collapsed.collapsed;
    return dedupeExtractedPlayers(collapsed.players, maxCount)
      .map((player) => ({ ...player, isTeammate }));
  };

  extractedData.teammates = sanitizePlayers(extractedData.teammates || [], 4, true);

  if (Array.isArray(extractedData.opponentTeams)) {
    extractedData.opponentTeams = extractedData.opponentTeams.map((team) => ({
      ...team,
      players: sanitizePlayers(team?.players || [], 4, false),
    }));
  }

  return {
    removedImplicitSelf,
    collapsedNearDuplicates,
  };
}
...
async function processCapture(imageBase64, activeUser = null, existingData = null, ocrMode = 'local', options = {}) {
  ...
  try {
    ...
    const parseScreenTypeHint = (value) => {
      const normalized = String(value || '').trim().toLowerCase();
      if (!normalized) return null;
      if (normalized === 'crewhub' || normalized === 'crew_hub') return SCREEN_TYPES.CREW_HUB;
      if (normalized === 'mapscreen' || normalized === 'map_screen' || normalized === 'tactical_map') return SCREEN_TYPES.MAP_SCREEN;
      return null;
    };
    const hintedScreenType = parseScreenTypeHint(typeof rawScreenTypeHint === 'string' ? rawScreenTypeHint : '');
    const priorCaptureScreenTypeHint = parseScreenTypeHint(
    ...
    let resolvedScreenType = screenDetection.type;
    if (hintedScreenType && hintedScreenType !== screenDetection.type) {
      resolvedScreenType = hintedScreenType;
      console.log(`[OCR] Screen type override from explicit hint: ${screenDetection.type} -> ${resolvedScreenType}`);
    } else if (priorCaptureScreenTypeHint && priorCaptureScreenTypeHint !== screenDetection.type) {
      const detectionConfidence = Number(screenDetection?.confidence || 0);
      if (detectionConfidence < 65) {
        resolvedScreenType = priorCaptureScreenTypeHint;
        console.log(
          `[OCR] Screen type override from prior capture (low confidence ${detectionConfidence}%): ` +
          `${screenDetection.type} -> ${resolvedScreenType}`
        );
      }
    }
    ...
    if (resolvedScreenType === SCREEN_TYPES.CREW_HUB) {
      ...
      const enemySlotKeys = ['enemyShips', 'enemyShips2', 'enemyShips3', 'enemyShips4'];
      const dedicatedSlotKeys = ['yourShip', ...enemySlotKeys];
      const dedicatedRegionThreshold = 0.15;
      let regionResult = null;
      const slotRegionResultsByKey = new Map();
      let mapScreenOcrInput = ocrResult;
      if (imageBuffer) {
        console.log('[OCR-Region] Running region-specific OCR for map teammate list');
      }
      if (imageBuffer) {
        const teammateRegionPromise = cropRegionAndOCR(
          imageBuffer,
          PLAYER_REGION,
        ...
        const slotRegionPromises = dedicatedSlotKeys.map(async (key) => {
          const slotRegion = activeRegions.mapScreen[key];
          if (!slotRegion) return { key, result: null };
          const result = await cropRegionAndOCR(
            imageBuffer,
            slotRegion,
            processed.originalWidth,
            processed.originalHeight,
            ...
            fontProfile,
            {
              threshold: dedicatedRegionThreshold,
              minWordConfidence: 8,
            }
          );
          return { key, result };
        });
        const [teammateResult, ...slotRegionEntries] = await Promise.all([
          teammateRegionPromise,
          ...slotRegionPromises,
        ]);
        regionResult = teammateResult;
        slotRegionEntries.forEach((entry) => {
          if (entry?.key) {
            slotRegionResultsByKey.set(entry.key, entry.result || null);
          }
          const slotWords = entry?.result?.words || [];
          if (slotWords.length === 0) return;
          const wordsPreview = slotWords
            .map((word) => String(word?.text || '').trim())
            .filter(Boolean)
            .join(' | ');
          console.log(
            `[OCR-Region] ${String(entry.key).toUpperCase()} region words (${slotWords.length}, threshold=${dedicatedRegionThreshold}): ${wordsPreview}`
          );
        });

        const scaleX = processed.originalWidth > 0 ? (processed.width / processed.originalWidth) : 1;
        const scaleY = processed.originalHeight > 0 ? (processed.height / processed.originalHeight) : 1;
        const supplementalWords = slotRegionEntries.flatMap((entry) => (
          Array.isArray(entry?.result?.words)
            ? entry.result.words.map((word) => ({
              ...word,
              bbox: {
                x0: Number(word?.bbox?.x0 || 0) * scaleX,
                y0: Number(word?.bbox?.y0 || 0) * scaleY,
                x1: Number(word?.bbox?.x1 || 0) * scaleX,
                y1: Number(word?.bbox?.y1 || 0) * scaleY,
              },
            }))
            : []
        ));
        if (supplementalWords.length > 0) {
          const dedupeKey = (word) => {
          ...
          const mergedWords = [...(ocrResult.words || []), ...supplementalWords];
      ...
      const enemyShipsList = Array.isArray(mapScreenData?.enemyShips) ? mapScreenData.enemyShips : [];
      const missingEnemyShipTypes = enemyShipsList.length < 4
        || enemyShipsList.some((ship) => !String(ship?.shipType || '').trim() || /^unknown$/i.test(String(ship?.shipType || '').trim()));
      if (imageBuffer && (missingYourShip || missingEnemyShipTypes)) {
        const retryRegions = [
          ...(missingYourShip ? ['yourShip'] : []),
          ...(missingEnemyShipTypes ? enemySlotKeys : []),
        ];
        const recoveredEnemySlots = new Map();

        for (const key of retryRegions) {
          const region = activeRegions.mapScreen[key];
          if (!region) continue;
          const regionThreshold = dedicatedRegionThreshold;
          const retryEntry = { slot: key, accepted: false, reason: 'no_improvement' };
          const cachedResult = slotRegionResultsByKey.get(key) || null;
          const retryResult = cachedResult || await cropRegionAndOCR(
            imageBuffer,
            region,
            processed.originalWidth,
            processed.originalHeight,
          ...
          if (!fromCache && retryWords.length > 0) {
            const wordsPreview = retryWords
              .map((word) => String(word?.text || '').trim())
              .filter(Boolean)
              .join(' | ');
            console.log(
              `[OCR-Region] ${String(key).toUpperCase()} region words (${retryWords.length}, threshold=${regionThreshold}): ${wordsPreview}`
            );
          }
          ...
          const teamText = retryText
            ...
            .replace(/\b(YOUR|SHIP|SHIPS|ENEMY|GUNSHIP|CREW|CREWSIZE|SIZE|HEALTH|KNOWN|HAZARDS|FEATURES|ARTIFACT|SPECIAL|RESOURCES|WILDGATE)\b/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();
          const teamName = teamText && looksLikeMapTeamName(teamText) && !isLikelyMapNoiseTeamName(teamText)
          ...
          if (key === 'yourShip') {
            ...
            const slotIndex = enemySlotKeys.indexOf(key);
            if (slotIndex >= 0 && (shipType || teamName)) {
              const slotColorFallback = ['red', 'orange', 'yellow', 'yellowgreen'][slotIndex] || 'unknown';
              recoveredEnemySlots.set(slotIndex, {
                teamName: teamName || '',
                shipType: shipType || '',
                color: slotColorFallback,
                confidence: shipType ? 74 : 66,
              });
              retryEntry.accepted = true;
              retryEntry.reason = shipType ? 'enemy_ship_recovered' : 'enemy_team_name_recovered';
            }
          }
          targetedRetries.push(retryEntry);
        }

        if (recoveredEnemySlots.size > 0) {
          const nextEnemyShips = Array.isArray(mapScreenData?.enemyShips) ? [...mapScreenData.enemyShips] : [];
          recoveredEnemySlots.forEach((candidate, slotIndex) => {
            const inSlot = nextEnemyShips[slotIndex];
            if (inSlot) {
              const slotShipType = String(inSlot.shipType || '').trim();
              const slotTeamName = String(inSlot.teamName || '').trim();
              const slotIsGenericName = /^enemy\s+team\s+\d+$/i.test(slotTeamName);
              nextEnemyShips[slotIndex] = {
                ...inSlot,
                teamName: (!slotTeamName || slotIsGenericName) ? (candidate.teamName || slotTeamName || `Enemy Team ${slotIndex + 1}`) : slotTeamName,
                shipType: slotShipType && !/^unknown$/i.test(slotShipType) ? slotShipType : (candidate.shipType || slotShipType || 'Unknown'),
                color: inSlot.color || candidate.color || 'unknown',
                teamColor: inSlot.teamColor || inSlot.color || candidate.color || 'unknown',
                confidence: Math.max(Number(inSlot.confidence || 0), Number(candidate.confidence || 0)),
              };
              return;
            }
            nextEnemyShips[slotIndex] = {
              teamName: candidate.teamName || `Enemy Team ${slotIndex + 1}`,
              shipType: candidate.shipType || 'Unknown',
              color: candidate.color || 'unknown',
              teamColor: candidate.color || 'unknown',
              players: [],
              confidence: Number(candidate.confidence || 66),
            };
          });
          mapScreenData.enemyShips = nextEnemyShips.filter(Boolean).slice(0, 4);
        }
      }
    ...
    const rosterSanitation = sanitizeExtractedPlayerCollections(extractedData, activeUser);
    if (rosterSanitation.removedImplicitSelf > 0) {
      console.log(`[OCR] Removed ${rosterSanitation.removedImplicitSelf} implicit active-user teammate entry from OCR output`);
    }
    if (rosterSanitation.collapsedNearDuplicates > 0) {
      console.log(`[OCR] Collapsed ${rosterSanitation.collapsedNearDuplicates} near-duplicate teammate name entry (prefix/edit-distance merge)`);
    }
