...
import { Plus, Minus, X, Users, Rocket, Skull, Star, Layout, Search, Trash2, Edit2, Filter, Pin, Clock, HeartCrack, Trophy, Scale, FileText } from 'lucide-react';
...
export const MissionPanel = ({ inputMode, setInputMode, timeMin, setTimeMin, timeSec, setTimeSec, damageTaken, setDamageTaken, poiEasy, setPoiEasy, poiMedium, setPoiMedium, poiEpic, setPoiEpic, selectedReachModifiers, toggleReachModifier, showArtifactSelect, setShowArtifactSelect, currentNote, setCurrentNote }: any) => (
  <div className="bg-md-sys-surface1 rounded-[32px] p-4 shadow-lg h-full overflow-y-auto custom-scrollbar flex flex-col">
      ...
      {inputMode === 'Manual' ? (
          <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                  <label className="text-xs font-black uppercase opacity-60 mb-2 block flex items-center gap-2"><FileText size={12}/> Mission Notes</label>
                  <textarea 
                      value={currentNote} 
                      onChange={(e) => setCurrentNote(e.target.value)} 
                      placeholder="Add brief notes about the mission..." 
                      className="w-full h-20 bg-md-sys-surface2 rounded-xl p-3 text-xs font-bold outline-none resize-none border border-transparent focus:border-md-sys-primary transition-all"
                  />
              </div>
          </div>
      ) : (
